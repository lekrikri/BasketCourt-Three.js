...
function initCannon() {
  ...
  const ballGeometry = new THREE.SphereBufferGeometry(ballShape.radius, 64, 64)

  const textureLoader = new THREE.TextureLoader();
  const ballTexture = textureLoader.load('/textures/balldimpled.png')
  const ballTextureNrm = textureLoader.load('/textures/balldimpled_nm.png')
  ...
  const ballMaterial = new THREE.MeshStandardMaterial({
    ...
    envMapIntensity: 1.4,
    map: ballTexture,
    normalMap: ballTextureNrm,
    
})
  //  ballMaterial.normalScale.set(-0.3, -0.3)
